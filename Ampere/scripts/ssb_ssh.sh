#!/bin/bash
#
# ssb_ssh <user@ip> <pass> <init_cmds|board_name> [serial_port#]
#
# Serial-to-Serial for BMC via SSH
#
...
case "$3" in
  [Mm][Tt]?(.)[Jj][Aa][Dd][Ee]) # mt.jade
    ...
    SSB_INIT='for i in 0 1 2 3; do gpiotool $((56+i)) --set-data-high; done'
    ;;
  *)
    SSB_BRDN="UserBoard"
    SSB_INFO="UserPorts"
...
for i in {0..3}; do
  socat pty,link=/dev/ttyS$((SSB_SERN+i)),raw exec:"sshpass -p $2 ssh $1 '$SSB_EXEC -s 115200 -D /dev/ttyS$i'" & j=$!
  eval "SSB_PID$i=$j"
done
sleep 1
for i in {0..3}; do
